...
const Form = () => {
  ...
  const acceptCondition=useRef(false);
  const [errorMessages, setErrorMessages] = useState({
    name: "",
    email: "",
    password: "",
    accept:false,
    // Add error states for other fields as needed
  });
  ...
  const validateForm = () => {
    const fields = [
      ...
      { ref: acceptCondition, name: "accept", message: "Please chex your condition ." }
    ...
    fields.forEach(({ ref, name, message }) => {
      ...
      const valueChexked=ref.current.checked;

      if (value === "") {
        errors[name] = message;
      } else if(valueChexked===false){
        errors[name]=message;
  ...
  return (
    ...
    <div className="flex">
      ...
      <div>
        ...
        <p className="text-sm">Thanks for inscription ,you are welcom to fammiley marAnime 😉</p>
      </div>
    </div>
  </div>
      ...
      <form className="w-full max-w-lg ml-8 mt-10" onSubmit={handleSubmit}>
    ...
    <label className="md:w-2/3 block text-slate-950 font-bold">
      <input className="mr-2 leading-tight" type="checkbox" name="chexInput" ref={acceptCondition}/>
